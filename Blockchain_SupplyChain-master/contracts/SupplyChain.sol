...
/********************************************** Transporter Section ******************************************/
    ...
    /*struct transinfo{
        ...
        mapping(address => address[]) transbatchidcid;
        mapping(address => uint[]) transbatchidtxtype;
        mapping(address => address[]) transbatchidpid;
    ...
    function loadConsingment ( 
        ...
        if(transportertype == 1) {  // Supplier to Manufacturer
            ...
            transbatchidcid[msg.sender].push(address(cid));
            transbatchidpid[msg.sender].push(address(pid));
            transbatchidtxtype[msg.sender].push(uint(transportertype));
    ...
    function transportCount() public view returns (uint count){
        require(
            UsersDetails[msg.sender].role == roles.transporter,
            "Only transporter Can call this function "
        );
        return transbatchidcid[msg.sender].length;
    }
   /* function gettransporterinfo(address User) public view returns(
        address cid1,
        uint transportertype1,
        address pid1
    ){
        return(
            transbatchid[User].cid1,
            transbatchid[User].transportertype1,
            transbatchid[User].pid1
        );
    }*/
    
    function getTransportBatchIdByIndex(uint index) public view returns(address cid,uint transportertype) {
        require(
            UsersDetails[msg.sender].role == roles.transporter,
            "Only transporter Can call this function "
        );
        return (transbatchidcid[msg.sender][index],transbatchidtxtype[msg.sender][index]);
    }
